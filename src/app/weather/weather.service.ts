...
export class WeatherService {
  ...
  public appID = '&appid=162b75f307ea43208fc584d47646e87e';
  ...
  getWeather(city, unit): Observable<any> {
    return this.http.get<any>(`${this.apiURL}${city}${this.appID}&units=${unit}`).pipe(catchError(this.handlError));
  }
  getLocalTime(lat, long): Observable<any> {
    return this.http.get<any>(`${this.timeApiURL}lat=${lat}&lng=${long}${this.userdetails}`);
  }
  handlError(error) {
